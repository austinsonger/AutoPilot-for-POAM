...
function fetchDataFromAPIs() {
  try {
    // Step 1: Fetch AWS Inspector Data (now secure)
    const awsData = fetchAWSInspectorData();
    // Step 2: Fetch Tenable Data
    const tenableData = fetchTenableData();
    // Step 3: Process and combine the data
    const combinedData = processVulnerabilityData(awsData, tenableData);
    // Step 4: Update Google Sheets
    updateGoogleSheet(combinedData);
    // Step 5: Apply conditional formatting
    applyConditionalFormatting();
    // Step 6: Update POAM workbook
    updatePOAMWorkbook(combinedData);
...
function fetchAWSInspectorData() { // AWS Fetch Function
    const awsCredentials = fetchAwsCredentialsFromGitHub(); // Fetch AWS credentials dynamically
    const requestOptions = {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${awsCredentials.AWS_ACCESS_KEY}`,
      },
    };
    const response = UrlFetchApp.fetch('https://<AWS_INSPECTOR_ENDPOINT_URL>', requestOptions);
  ...
  function fetchTenableData() { // Tenable Fetch Function
    const tenableApiKey = fetchTenableApiKeyFromGitHub(); // Fetch Tenable API key dynamically
    const requestOptions = {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${tenableApiKey}`,
      },
    };
    const response = UrlFetchApp.fetch('https://<TENABLE_ENDPOINT_URL>', requestOptions);
    const responseText = response.getContentText();
    if (C_USEFUL.isJson(responseText)) {
      return JSON.parse(responseText);
    } else {
